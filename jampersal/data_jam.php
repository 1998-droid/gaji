...
<span class="glyphicon glyphicon-upload"></span> Import Data
			...
			<form method="post" action="del.php" id="form-delete">
                ...
                <tbody>
					...
					$bulan = mysqli_query($connect, "SELECT tgl FROM tb_jam where tgl='2021-07-10' order by tgl asc");
					...
					Farmasi_langsung)-(Pelayanan_pi+Farmasi_pi+Pelayanan_pl+Farmasi_pl)-(Hari_Raya+Kurban+POT_BINA_SEHAT+Lain2)) as neto
					 FROM tb_jam order by tgl asc") 
					or die (mysqli_error($connect));
					
					while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
						echo "<tr>";
						echo "<td><input type='checkbox' class='check-item' name='nis[]' value='".$data['id_jam']."'></td>";
						...
						echo "<td>".'Rp. '.number_format($data['Pelayanan_langsung'])."</td>";
						echo "<td>".'Rp. '.number_format($data['Farmasi_langsung'])."</td>";
						echo "<td>".'Rp. '.number_format($data['total_indeks'])."</td>";
						echo "<td>".($data['Pelayanan_pi'])."</td>";
						echo "<td>".($data['Farmasi_pi'])."</td>";
						echo "<td>".'Rp. '.number_format($data['Pelayanan_pl'])."</td>";
						echo "<td>".$data['Farmasi_pl']."</td>";
						echo "<td>".'Rp. '.number_format($data['t_pph'])."</td>";
						echo "<td>".'Rp. '.number_format($data['bersih'])."</td>";
						...
						echo "<td>".'Rp. '.number_format($data['t_pot'])."</td>";
						echo "<td>".'Rp. '.number_format($data['neto'])."</td>";
