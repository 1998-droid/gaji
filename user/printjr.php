...
$sql_2 = mysqli_query($connect, "SELECT *, a.nama, b.nip, a.tgl, (gol+gr+MK+TJ+JAB+TF+LL) AS tot,
 ...
 (Pelayanan_pi+Farmasi_pi+Pelayanan_pl+Farmasi_pl) as t_pph, ((Pelayanan_indeks+Farmasi_indeks+Pelayanan_langsung+
					...
					Farmasi_langsung)-(Pelayanan_pi+Farmasi_pi+Pelayanan_pl+Farmasi_pl)-(Hari_Raya+Kurban+POT_BINA_SEHAT+RUANGAN+ARISAN_KARU+POT_ARTHA_MEDIKA+Lain2)) as neto FROM tb_jr a 
JOIN tb_user b ON b.nip=a.nip WHERE id_jr ='$id'") 
...
<html>
<head>
    <title>Form Slip Jasa Raharja</title>
</head>
<body>
...
<tr>
            <td  width="1">Tahun Pelayanan</td>
            <td width="1">:</td>
            <td > <?php echo date('Y', strtotime($data['tgl'])); ?></td><br>
        </tr>
